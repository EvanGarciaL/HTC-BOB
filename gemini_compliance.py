...
def evaluate_opportunity_with_ai(product_name: str, category: str):
    ...
    if API_QUOTA_EXHAUSTED or not genai or not os.environ.get("GEMINI_API_KEY"):
    ...
    try:
        ...
        print(
            f"Error during AI evaluation for {product_name}, returning DEMO data due to API deprecation: {e}")
        # Return high-quality mock data for the UX/Dashboard demonstration
        is_high_risk = "pudding" in product_name.lower(
        ) or "supplement" in product_name.lower()
        return {
...
def get_competitors_and_recommendations(product_name: str, category: str) -> dict:
    ...
    if API_QUOTA_EXHAUSTED or not genai or not os.environ.get("GEMINI_API_KEY"):
        raise EnvironmentError(
            "Gemini AI is not configured, GEMINI_API_KEY is missing, or quota is exhausted.")
    ...
    for attempt in range(2):
        try:
            ...
            if not raw:
                raise ValueError(
                    f"Empty response from Gemini for '{product_name}'")

            matches = re.findall(r"\{.*?\}", raw, re.DOTALL)
            if not matches:
                raise ValueError(
                    f"No JSON object found in Gemini response for '{product_name}'")
            ...
            if result is None:
                raise ValueError(
                    f"[PARSE ERROR] {product_name}: no valid JSON object found")
            ...
            if not isinstance(competitors, list) or not isinstance(recommendations, list):
                raise ValueError(
                    f"[VALIDATION ERROR] {product_name}: competitors or recommendations is not a list")

            if not all(isinstance(x, str) for x in competitors):
                raise ValueError(
                    f"[VALIDATION ERROR] {product_name}: non-string value in competitors")
            if not all(isinstance(x, str) for x in recommendations):
                raise ValueError(
                    f"[VALIDATION ERROR] {product_name}: non-string value in recommendations")

            if not (3 <= len(competitors) <= 5):
                raise ValueError(
                    f"[VALIDATION ERROR] {product_name}: competitors length {len(competitors)} out of bounds (3–5)")
            if not (4 <= len(recommendations) <= 6):
                raise ValueError(
                    f"[VALIDATION ERROR] {product_name}: recommendations length {len(recommendations)} out of bounds (4–6)")

            if not all(_is_valid_entry(x) for x in competitors):
                raise ValueError(
                    f"[VALIDATION ERROR] {product_name}: malformed competitor entries")
            if not all(_is_valid_entry(x) for x in recommendations):
                raise ValueError(
                    f"[VALIDATION ERROR] {product_name}: malformed recommendation entries")
            ...
            if '429' in str(e) or 'RESOURCE_EXHAUSTED' in str(e) or '503' in str(e) or 'UNAVAILABLE' in str(e):
                API_QUOTA_EXHAUSTED = True
                break  # Stop trying if we hit rate limits or model is overloaded
            if attempt == 0:
                print(
                    f"[RETRY] {product_name} attempt {attempt + 1} failed: {e}")
